...
class PHP_ParserGenerator_Data
{
     ...
     */
    public const NO_OFFSET = -2147483647;
    /**
            ...
            if ($sp->type == PHP_ParserGenerator_Symbol::MULTITERMINAL) {
                for ($i = 0; $i < $sp->nsubsym; $i++) {
                    PHP_ParserGenerator_Action::Action_add(
                        $stp->ap,
                        PHP_ParserGenerator_Action::SHIFT,
                        $sp->subsym[$i],
                        $newstp
                    );
                }
                    ...
                    for ($j = 0; $j < $this->nterminal; $j++) {
                        if (isset($cfp->fws[$j])) {
                            ...
                            ** rule "cfp->rp" if the lookahead symbol is "$this->symbols[j]" */
                            PHP_ParserGenerator_Action::Action_add(
                                $stp->ap,
                                PHP_ParserGenerator_Action::REDUCE,
                                $this->symbols[$j],
                                $cfp->rp
                            );
                        }
...
//        struct axset *ax;
        ...
        $this->tplt_print(
            $out,
            $this->declare_classcode,
            $this->declare_classln,
            $lineno
        );
        $this->tplt_xfer($this->name, $in, $out, $lineno);
        ...
        $this->tplt_print(
            $out,
            $this->include_classcode,
            $this->include_classln,
            $lineno
        );
        $this->tplt_xfer($this->name, $in, $out, $lineno);
