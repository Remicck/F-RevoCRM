...
class Google_Contacts_Connector extends WSAPP_TargetConnector
{
    ...
    public const CONTACTS_URI = 'https://www.google.com/m8/feeds/contacts/default/full';

    public const CONTACTS_GROUP_URI = 'https://www.google.com/m8/feeds/groups/default/full';

    public const CONTACTS_BATCH_URI = 'https://www.google.com/m8/feeds/contacts/default/full/batch';
    ...
    protected function addEntityDetailsToAtomEntry(&$entry, $entity, $user)
    {
        ...
        if ($entity->get('salutationtype')) {
            $gdName->addChild("namePrefix", Google_Utils_Helper::toGoogleXml($entity->get('salutationtype')), $gdNS);
        }
        if ($entity->get('firstname')) {
            $gdName->addChild("givenName", Google_Utils_Helper::toGoogleXml($entity->get('firstname')), $gdNS);
        }
        if ($entity->get('lastname')) {
            $gdName->addChild("familyName", Google_Utils_Helper::toGoogleXml($entity->get('lastname')), $gdNS);
        }
        $gdRel = $gdNS . '#';
        ...
        if ($entity->get('account_id') || $entity->get('title')) {
            ...
            if ($entity->get('account_id')) {
                $gdOrganization->addChild("orgName", Google_Utils_Helper::toGoogleXml($entity->get('account_id')), $gdNS);
            }
            if ($entity->get('title')) {
                $gdOrganization->addChild("orgTitle", Google_Utils_Helper::toGoogleXml($entity->get('title')), $gdNS);
            }
        }
        ...
        foreach ($this->fieldMapping as $vtFieldName => $googleFieldDetails) {
            ...
            switch ($googleFieldDetails['google_field_name']) {
                    ...
                    if ($vtFieldName == 'mailingaddress') {
                        if ($entity->get('mailingstreet') || $entity->get('mailingpobox') || $entity->get('mailingzip') ||
                            ...
                            if ($entity->get('mailingstreet')) {
                                $gdAddressHome->addChild("street", Google_Utils_Helper::toGoogleXml($entity->get('mailingstreet')), $gdNS);
                            }
                            if ($entity->get('mailingpobox')) {
                                $gdAddressHome->addChild("pobox", Google_Utils_Helper::toGoogleXml($entity->get('mailingpobox')), $gdNS);
                            }
                            if ($entity->get('mailingzip')) {
                                $gdAddressHome->addChild("postcode", Google_Utils_Helper::toGoogleXml($entity->get('mailingzip')), $gdNS);
                            }
                            if ($entity->get('mailingcity')) {
                                $gdAddressHome->addChild("city", Google_Utils_Helper::toGoogleXml($entity->get('mailingcity')), $gdNS);
                            }
                            if ($entity->get('mailingstate')) {
                                $gdAddressHome->addChild("region", Google_Utils_Helper::toGoogleXml($entity->get('mailingstate')), $gdNS);
                            }
                            if ($entity->get('mailingcountry')) {
                                $gdAddressHome->addChild("country", Google_Utils_Helper::toGoogleXml($entity->get('mailingcountry')), $gdNS);
                            }
                        }
                    } else {
                        if ($entity->get('otherstreet') || $entity->get('otherpobox') || $entity->get('otherzip') ||
                            ...
                            if ($entity->get('otherstreet')) {
                                $gdAddressWork->addChild("street", Google_Utils_Helper::toGoogleXml($entity->get('otherstreet')), $gdNS);
                            }
                            if ($entity->get('otherpobox')) {
                                $gdAddressWork->addChild("pobox", Google_Utils_Helper::toGoogleXml($entity->get('otherpobox')), $gdNS);
                            }
                            if ($entity->get('otherzip')) {
                                $gdAddressWork->addChild("postcode", Google_Utils_Helper::toGoogleXml($entity->get('otherzip')), $gdNS);
                            }
                            if ($entity->get('othercity')) {
                                $gdAddressWork->addChild("city", Google_Utils_Helper::toGoogleXml($entity->get('othercity')), $gdNS);
                            }
                            if ($entity->get('otherstate')) {
                                $gdAddressWork->addChild("region", Google_Utils_Helper::toGoogleXml($entity->get('otherstate')), $gdNS);
                            }
                            if ($entity->get('othercountry')) {
                                $gdAddressWork->addChild("country", Google_Utils_Helper::toGoogleXml($entity->get('othercountry')), $gdNS);
                            }
                        }
                    }
                    break;
                case 'content':
                    if ($entity->get($vtFieldName)) {
                        $entry->addChild('content', Google_Utils_Helper::toGoogleXml($entity->get($vtFieldName)));
                    }
                    break;
                case 'gContact:userDefinedField':
                ...
                if (count($xmlList)) {
                    foreach ($xmlList as $tagDetails) {
                        ...
                        if (isset($googleContactId) && isset($googleContactGroupId)) {
                            ...
                            unset($googleContactId);
                            unset($googleContactGroupId);
                        }
                    }
                }
            }
