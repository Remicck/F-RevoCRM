...
class EmailTemplate
{
    ...
    public function process($params, $depth=0)
    {
            ...
            if (count($tableList) > 0 && count($columnListTable) > 0) {
                ...
                foreach ($tableList as $index => $tableName) {
                    if ($tableName != $tableList[0]) {
                        if ($tableName == 'vtiger_seactivityrel' || $tableName == 'vtiger_cntactivityrel' || $tableName == 'vtiger_crmentity_user_field' || $tableName == 'vtiger_invoice_recurring_info') {
                            $sql .= ' LEFT JOIN ';
                        } else {
                            $sql .= ' INNER JOIN ';
                        }
                    ...
                    if (isset($values[$fieldName]) &&
                        ...
                        if (strcasecmp($webserviceField->getFieldDataType(), 'reference') === 0) {
                            ...
                            if (!$this->isProcessingReferenceField($params) && !empty($values[$fieldName])) {
                                $this->process(array('parentMeta' => $meta, 'referencedMeta' => $referencedObjectMeta, 'field' => $fieldName, 'id' => $values[$fieldName]), $depth+1);
                            }
                            $values[$fieldName] =
                            ...
                            */
                            if (in_array($fieldColumnMapping[$fieldName], array_keys($this->referencedFields))) {
                                $this->process(array('parentMeta' => $meta, 'referencedMeta' => $referencedObjectMeta, 'field' => $fieldName, 'id' => $values[$fieldName], 'owner' => true), $depth+1);
                            }
                            ...
                            if ($referenceColumn == 'contactid' && $this->module == 'Events') {
                                $$this->multiRefValues[$fieldName][] = $values[$fieldName];
                            } else {
