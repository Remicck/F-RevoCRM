...
class DateTimeField
{
    ...
    public function getDisplayTimeWithoutSec($user = null)
    {
        global $log;
        $log->debug("Entering getDisplayTimeWithoutSec(" . $this->datetime . ") method ...");
        $date = self::convertToUserTimeZone($this->datetime, $user);
        $time = $date->format("H:i");
        $log->debug("Exiting getDisplayTimeWithoutSec method ...");
        return $time;
    }
        ...
        if ($user->date_format) {
            ...
            if (!empty($date)) {
                switch ($user->date_format) {
                    ...
                    case 'dd/mm/yyyy': list($d, $m, $y) = explode('/', $date);
                    break;
                    case 'mm/dd/yyyy': list($d, $m, $y) = explode('/', $date);
                    break;
                    case 'mm-dd-yyyy': list($m, $d, $y) = explode('-', $date);
                    break;
                    case 'dd-mm-yyyy': list($d, $m, $y) = explode('-', $date);
