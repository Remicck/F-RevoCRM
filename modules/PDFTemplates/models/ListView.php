...
class PDFTemplates_ListView_Model extends Vtiger_ListView_Model
{
    private $querySelectColumns = array('templatename', 'foldername', 'systemtemplate', 'module', 'description');
    private $listViewColumns = array('templatename', 'description', 'module');
    ...
    public function getListViewMassActions($linkParams)
    {
        ...
        $massActionLinks[] = array(
            ...
            'linkurl' => 'javascript:PDFTemplates_List_Js.massDeleteRecords("index.php?module='.$moduleModel->get('name').'&action=MassDelete");',
            'linkicon' => ''
        );
    ...
    {
        $fieldObjects = array();
        $listViewHeaders = array('Template Name' => 'templatename', 'Description' => 'description', 'Module Name' => 'module');
        foreach ($listViewHeaders as $key => $fieldName) {
            $fieldModel = new PDFTemplates_Field_Model();
    ...
    public function getListViewEntries($pagingModel)
    {
        ...
        $moduleActiveCheck = '(vtiger_tab.presence IN (0,2) OR vtiger_pdftemplates.module IS NULL OR vtiger_pdftemplates.module = "")';
        $listQuery .= $whereQuery. $moduleActiveCheck;
        //To retrieve only selected module records
        if ($sourceModule) {
            $listQuery .= " AND vtiger_pdftemplates.module = ?";
            $params[] = $sourceModule;
        }
        ...
        for ($i = 0; $i < $num_rows; $i++) {
            ...
            $recordModel->setModule('PDFTemplates');
    ...
    {
        $listQuery = 'SELECT templateid,'.implode(',', $this->querySelectColumns).' FROM vtiger_pdftemplates
						LEFT JOIN vtiger_tab ON vtiger_tab.name = vtiger_pdftemplates.module
