...
class PDFTemplates_Record_Model extends Vtiger_Record_Model
{
    /**
     ...
     * To get the fields from module, which has the pdf field
     * @return <arrray> template fields
     */
    public function getPDFTemplateFields()
    {
        return $this->getModule()->getAllModulePDFTemplateFields();
    }

    /**
     * Function to get the PDF Template Record
     * @param type $record
     * @return <PDFTemplate_Record_Model>
     */
    ...
    public static function getInstanceById($templateId, $module=null)
    {
        ...
        $sql = 'SELECT * FROM vtiger_pdftemplates WHERE templateid = ?';
        $params = array($templateId);
        $result = $db->pquery($sql, $params);
        if ($db->num_rows($result) > 0) {
            $row = $db->query_result_rowdata($result, 0);
            $recordModel = new self();
            return $recordModel->setData($row)->setModule('PDFTemplates');
        }
        throw new Exception(vtranslate('LBL_RECORD_DELETE', 'Vtiger'), 1);
    }

    public static function getAllForPDFTask($moduleName)
    {
        $db = PearDatabase::getInstance();
        $sql = 'SELECT templateid, templatename, body, deleted FROM vtiger_pdftemplates WHERE module = ? ORDER BY templateid DESC';
        ...
        if ($numRows > 0) {
            for ($i=0;$i<$numRows;$i++) {
                ...
                $recordModel = new self();
                $recordModels[$row['templateid']] = $recordModel->setData($row)->setModule('PDFTemplates');
            }
        }
        return $recordModels;
    }
    ...
    {
        $db = PearDatabase::getInstance();
        $sql = 'SELECT * FROM vtiger_pdftemplates WHERE subject = ? AND systemtemplate = ?';
        ...
        if ($numRows > 0) {
            $row = $db->query_result_rowdata($result, 0);
            $recordModel = new self();
            $recordModel->setData($row)->setModule('PDFTemplates');
            return $recordModel;
        }
    }
    ...
    public function updateImageName($imageName)
    {
        $db = PearDatabase::getInstance();
        if (!empty($imageName)) {
            $db->pquery('UPDATE vtiger_pdftemplates SET templatepath=? WHERE templateid=?', array($imageName . ".png", $this->getId()));
        }
    }
}
