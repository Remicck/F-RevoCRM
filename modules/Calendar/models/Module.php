...
class Calendar_Module_Model extends Vtiger_Module_Model
{
    ...
    public function getRelatedHistoryListFields()
    {
        $entityInstance = CRMEntity::getInstance($this->getName());
        $list_fields = $entityInstance->list_fields;
        $list_fields_name = $entityInstance->list_fields_name;
        ...
        foreach ($list_fields as $key => $fieldInfo) {
            foreach ($fieldInfo as $columnName) {
                if (array_key_exists($key, $list_fields_name)) {
                    if ($columnName == 'lastname' || $columnName == 'activity' || $columnName == 'recurringtype') {
                        continue;
                    }
                    if ($columnName == 'status') {
                        $relatedListFields[$columnName] = 'taskstatus';
    ...
    */
    public static function getSharedUsersOfCurrentUser($id)
    {
        ...
        if ($currentUser->isAdminUser()) {
            ...
            $query = "SELECT vtiger_users.last_name, vtiger_users.first_name, vtiger_users.id AS userid
        ...
        for ($i=0; $i<$rows; $i++) {
            ...
            $userName = $db->query_result($result, $i, 'last_name').' '.$db->query_result($result, $i, 'first_name');
            $userIds[$id] =$userName;
        }
    ...
    */
    public static function getSharedUsersInfoOfCurrentUser($id = null)
    {
        $db = PearDatabase::getInstance();
        ...
        $query = "SELECT
				...
				FROM
					vtiger_users u
					LEFT OUTER JOIN vtiger_shareduserinfo s ON s.shareduserid = u.id and s.userid = ?
				WHERE
					u.status = 'Active'
    ...
    */
    ...
    public static function getDateFieldModulesList()
    {
        ...
        foreach ($eventFieldsList as $fieldName) {
            $fieldInstance = Vtiger_Field_Model::getInstance($fieldName, $moduleInstance);
            if ($fieldInstance) {
                $eventFieldLabelsList[$fieldName] = vtranslate($fieldInstance->label, 'Events');
            }
        }
        ...
        foreach ($calendarFieldsList as $fieldName) {
            ...
            if ($fieldModel) {
                $calendarFieldLabelsList[$fieldName] = vtranslate($fieldModel->label, 'Calendar');
            }
        }
    ...
    public function getCalendarViewTypesToAdd($userId)
    {
    ...
    public function getVisibleCalendarViewTypes($userId)
    {
        $db = PearDatabase::getInstance();
    ...
    */
    ...
    /**
    * To get the lists of user list in shared calendar.
    * @param $id --  user id
    * @returns <Array> $userList
    */
    public static function getSharedUserList($id = null)
    {
        ...
        for ($i=0; $i<$rows; $i++) {
            ...
            $userName = $adb->query_result($result, $i, 'last_name').' '.$adb->query_result($result, $i, 'first_name');
