...
//	   'Recurring Type'=>'recurringtype',
    ...
    public $mandatory_fields = array('createdtime', 'modifiedtime', 'subject', 'assigned_user_id','date_start','due_date','eventstatus','taskstatus','activitytype','reminder_time','recurringtype');
    ...
    public $is_not_invitees_update = false;
    public $invitee_parentid = null;
    public $invitee_array = null;

    public $is_allday = null;

    // 招待書にコピーしないフィールドの一覧を定義（更新時のみ）
    public $notCopyFields = array(
        'id',
        'selectedusers',
        'assigned_user_id',
        'mode',
    ...
    public function save_module($module)
    {
        ...
        } elseif (empty($this->invitee_array) && count($_REQUEST['selectedusers']) > 0) {// 概要欄や関連からの遷移の場合
...
public function insertIntoRecurringTable(& $recurObj)
{
    ...
    public function insertIntoSmActivityRel($module, $invitees_array)
    {
        global $adb;
        global $current_user;
        if ($this->mode == 'edit') {
