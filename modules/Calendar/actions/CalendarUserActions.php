...
class Calendar_CalendarUserActions_Action extends Vtiger_Action_Controller
{
    ...
    /**
     * Function to get user list by id and target
     * @param Vtiger_Request $request
     * @return Vtiger_Response $response
     */
    public function getUserList(Vtiger_Request $request)
    {
        ...
        if ($target == 'Roles') {
            $role = Settings_Roles_Record_Model::getInstanceById($id);
            $users = $role->getUserNameList();
        } elseif ($target == 'Groups') {
            $group = Settings_Groups_Record_Model::getInstance($id);
            $users = $group->getUserNameList();
        } else {
            // default my user list.
            $users = Calendar_Module_Model::getSharedUserList();
        }

        $info = Calendar_Module_Model::getSharedUsersInfoOfCurrentUser();

        $result = array('users' => $users, 'sharedinfo' => $info);
        $response = new Vtiger_Response();
        $response->setResult($result);
        $response->emit();
    }
