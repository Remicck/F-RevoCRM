...
class Settings_Groups_Record_Model extends Settings_Vtiger_Record_Model
{
   ...
   public static function getInstanceByName($name, $excludedRecordId = array())
   {
    ...
    {
        $users = $this->getUsersList(false);//get $this->members before call this.
        $userNameList = array();

        foreach ($this->members['Users'] as $id => $user) {
            if ($user->get('status') == 'Active') {
                $userNameList[$user->get("userId")] = $user->get('name');
            }
        }

        foreach ($users as $uid => $usermodel) {
            if ($usermodel->get('status') == 'Active') {
                $userNameList[$uid] = $usermodel->getName();
            }
        }
        return $userNameList;
    }
