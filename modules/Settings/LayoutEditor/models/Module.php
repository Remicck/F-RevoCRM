...
class Settings_LayoutEditor_Module_Model extends Vtiger_Module_Model
{
    ...
    public function addField($fieldType, $blockId, $params)
    {
        ...
        if ($fieldType == 'Reference') {
            $fieldModel->setRelatedModules($params['relationmodule']);

            $db->pquery("DELETE FROM vtiger_relatedlists WHERE relationfieldid = ?", array($fieldModel->getId()));
            $module = Vtiger_Module::getInstance($fieldModel->getModuleName());
            foreach ($params['relationmodule'] as $relModuleName) {
                $relModule = Vtiger_Module::getInstance($relModuleName);
                $relModule->setRelatedList($module, vtranslate($fieldModel->getModuleName(), $relModule->name)."_".$fieldModel->get('label'), array('ADD'), 'get_dependents_list', $fieldModel->getId());
            }
        }
        return $fieldModel;
    }
