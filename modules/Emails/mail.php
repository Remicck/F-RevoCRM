...
function addSignature($contents, $fromname, $fromEmail = '')
{
    ...
    if ($sign == null) {
        ...
        VTCacheUtils::setUserFullName($fromname, $adb->query_result($result, 0, "last_name").' '.$adb->query_result($result, 0, "first_name"));
    }
...
function setMailerProperties($mail, $subject, $contents, $from_email, $from_name, $to_email, $attachment='', $emailid='', $module='', $logo='')
{
    ...
    if (empty($userFullName)) {
        $rs = $adb->pquery("select last_name,first_name from vtiger_users where user_name=?", array($from_name));
